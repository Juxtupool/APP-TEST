...
async function connectSerial(silent = false) {
    ...
    // Find Monolith (or generic RP2040) port or default to first
    let portToConnect = ports[0][0];
    ...
    const isConnectedResult = result.status === 'success' || result.connected === true || (result.data && result.data.connected === true);

        ...
        try:
            ...
            if connected:
                ...
                if not api._serial_service.check_physical_connection_health():
                    print("Tray Monitor: Detected physical device disconnection")
                    # UI update and tray refresh handled by callbacks in serial_service/api
                    refresh_tray(api)
            else:
                # Disconnected: Periodically scan to refresh tray and detect new devices
                refresh_tray(api)
        except Exception as e:
            print(f"Error in tray loop: {e}")
        time.sleep(5)

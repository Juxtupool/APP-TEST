    ...
    document.getElementById('btn-check-updates').addEventListener('click', async () => {
        ...
        try {
            ...
            if (res.status === 'success') {
                ...
                // App Status
                if (app.error) {
                    msg += `<strong>App:</strong> Error - ${app.error}`;
                } else if (app.update_available) {
                    msg += `<strong>App:</strong> Update Available (v${app.latest_version})<br>`;
                    if (app.html_url) {
                        msg += `<small>Download from <a href="#" onclick="pywebview.api.generate_macro_submission_url({'url': '${app.html_url}'})">GitHub</a></small>`;
                    }
                } else {

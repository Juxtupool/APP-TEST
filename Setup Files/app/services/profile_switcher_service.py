...
class ProfileSwitcherService:
    ...
    def get_active_windows(self) -> list:
        """Returns a list of unique process names from currently visible windows."""
        process_names = set()
        
        def callback(hwnd, names_set):
            if win32gui.IsWindowVisible(hwnd):
                title = win32gui.GetWindowText(hwnd)
                # Filter out windows with empty titles or certain system windows if needed
                if title:
                    try:
                        _, pid = win32process.GetWindowThreadProcessId(hwnd)
                        proc = psutil.Process(pid)
                        names_set.add(proc.name())
                    except (psutil.NoSuchProcess, psutil.AccessDenied):
                        pass
            return True
        
        try:
            # win32gui.EnumWindows requires a callback

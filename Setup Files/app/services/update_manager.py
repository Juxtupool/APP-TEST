...
class UpdateManager:
    ...
    def check_app_update(self):
        ...
        try:
            ...
            current_version = self.config.get('app', {}).get('current_version', '0.0.0')
            ...
            # Priority: 
            # 1. Target extension (.exe for frozen)
            # 2. .zip (backup)
            # 3. Source code zipball (fallback)
            
            preferred_ext = '.exe' if getattr(sys, 'frozen', False) else '.zip'
            backup_ext = '.zip'
            ...
            # Fallback to source code if still nothing found
            if not download_url:
    ...
    def _restart_sequence(self):
        ...
        try:
            ...
            if is_frozen:
                ...
                wait_cmd = f'taskkill /PID {current_pid} /F' # Force kill self if script doesn't
                # Actually, we rely on sys.exit(), but bat can wait
            else:
